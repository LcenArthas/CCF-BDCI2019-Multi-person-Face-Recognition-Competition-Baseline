...
def get_featurs(model_dic, test_list):

    device = torch.device("cuda")

    pbar = tqdm(total=len(test_list))
    for idx, img_path in enumerate(test_list):
        model = model_dic[img_path.split('/')[-1].split('_')[0]]
        pbar.update(1)
...
for model_path in model_path:
    ...
    model.eval()
    model_dic[model_path.split('_')[1]] = model
...
s = time.time()
features = get_featurs(model_dic, img_paths)
t = time.time() - s
